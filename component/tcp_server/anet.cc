...
int anetUnixServer(char *err, char *path, mode_t perm, int backlog) {
  ...
  sa.sun_family = AF_LOCAL;
  strncpy(sa.sun_path, path, sizeof(sa.sun_path) - 1);
  if (anetListen(err, s, (struct sockaddr *)&sa, sizeof(sa), backlog, perm) ==
      ANET_ERR)
    return ANET_ERR;
  return s;
}
...
int anetTcpAccept(char *err, int serversock, char *ip, size_t ip_len,
  ...
  if ((fd = anetGenericAccept(err, serversock, (struct sockaddr *)&sa,
                              &salen)) == ANET_ERR)
    return ANET_ERR;
...
int anetPipe(int fds[2], int read_flags, int write_flags) {
   ...
   * both ends. There is no harm to set O_CLOEXEC to prevent fd leaks. */
  ...
  /* When we reach here with pipe_flags of 0, it means pipe2 failed (or was not
   * attempted), so we try to use pipe. Otherwise, we skip and proceed to set
