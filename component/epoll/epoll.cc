...
AeEventLoop::AeEventLoop(int size)
   ...
   * vector with it. */
  for (int i = 0; i < size; i++) events_[i].mask = AE_NONE;
...
int AeEventLoop::aeResizeSetSize(int size) {
   ...
   * an AE_NONE mask. */
  for (int i = maxfd_ + 1; i < size; i++) events_[i].mask = AE_NONE;
  return AE_OK;
}
...
int AeEventLoop::aeCreateFileEvent(int fd, int mask, aeFileProc proc,
  ...
  aeFileEvent *fe = &events_[fd];

  if (aeApiAddEvent(fd, mask) == -1) return AE_ERR;
  fe->mask |= mask;
...
int AeEventLoop::aeProcessEvents() {
  ...
  for (int j = 0; j < numevents; j++) {
    ...
    if (!invert && fe->mask & mask & AE_READABLE) {
      ...
      fired_num++;
      fe = &events_[fd]; /* Refresh in case of resize. */
    }
...
int AeEventLoop::aeApiPoll(struct timeval *tvp) {
  ...
  retval =
      epoll_wait(epfd_, &epoll_events_[0], event_size_,
                 tvp ? (tvp->tv_sec * 1000 + (tvp->tv_usec + 999) / 1000) : -1);
  if (retval > 0) {
    numevents = retval;
    for (int j = 0; j < numevents; j++) {
      int mask = 0;
      struct epoll_event *e = &epoll_events_[j];
